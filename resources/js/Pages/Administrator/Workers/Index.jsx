...
const WorkersIndex = ({ workers, companies }) => {
    ...
    useEffect(() => {
        ...
        // Filter by name
        if (searchTerm) {
            results = results.filter((worker) => worker.name.toLowerCase().includes(searchTerm.toLowerCase()))
        }

        // Filter by company
        if (selectedCompany) {
            results = results.filter((worker) => worker.company_id === Number.parseInt(selectedCompany))
        }
                    ...
                    <div className="p-6 bg-gray-50 border-b border-gray-200">
                      ...
                      {selectedCompany
                                        ...
                                        <div className="p-2 max-h-60 overflow-y-auto">
                                            ...
                                            {companies.map((company) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-3 py-2 hover:bg-gray-100 rounded ${Number.parseInt(selectedCompany) === company.id ? "bg-blue-50 text-blue-700" : ""}`}
                                                >
                                                    {company.name}
                                                </button>
                                            ))}
                        ...
                        <div className="mt-4 text-sm text-gray-600">
                            {filteredWorkers.length} {filteredWorkers.length === 1 ? "treballador trobat" : "treballadors trobats"}
                        </div>
                    </div>
                                ...
                                <div key={worker.id} className="p-6 hover:bg-gray-50 transition-colors duration-150">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="flex-grow">
                                            <h3 className="text-xl font-semibold text-gray-800 mb-1">{worker.name}</h3>
            ...
            {showDialog && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full">
                        <h2 className="text-xl font-bold text-gray-800 mb-4">Eliminar Treballador</h2>
